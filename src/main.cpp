...
pthread_t           threads[THREAD_MAX];  //for multi threading
...
float   r       = 23.0,     /* circle "r" */
        d       = 0.3,      /* cuboid width / 2 */
        deg     = 15.0,
        curtime;
double  SAMPLE_COUNT,
        SAMPLE_RATE;
bool    th_join_stat = false;
...
void loadData(){
    ...
    std::cout   << "SampleRate: " << (SAMPLE_RATE = buffer.getSampleRate()) 
                << std::endl;
    std::cout   << "SampleCount: " << (SAMPLE_COUNT = buffer.getSampleCount()) 
                << std::endl;
    std::cout   << "SampleCount / 2: " << buffer.getSampleCount() / 2 
                << std::endl;
    std::cout   << "expected no of samples: " 
                << (((int)(buffer.getSampleCount()/N))*N) / 2 << std::endl;
    std::cout   << "channel: " << buffer.getChannelCount() << std::endl;
    std::cout   << "Duration: " << buffer.getDuration().asSeconds() 
                << " seconds" << std::endl;
    ...
    double  mag[N / 2],
            sf              = buffer.getSampleRate(),
            roof            = buffer.getSampleCount(),
            framePointer    = 0;
    ...
    {
        for (i = framePointer, j = 0; 
              ((i < (framePointer + N)) && (framePointer < (roof - N))); 
              i++, j++)
        {
    ...
    std::cout << "actual no of samples: " << ampdb.size() << std::endl;
...
void display(void)
{
    ...
    else
    {
        ...
        float               tottime     = buffer.getDuration().asSeconds(),
                            cursor      = sound.getPlayingOffset().asSeconds(),
                            timepercent = (cursor / tottime)*760;
...
//            }
    ...
    if ((loading != 0) && (th_join_stat == false))
    {
        int err_no;
        ...
        {
            std::cout   << "FILE: " << __FILE__ << "FUNCTION: " << __FUNCTION__ 
